...
def compute_fvd(real_dir, fake_dir, frames=16, batch_size=1, device="cuda"):
    ...
    with torch.no_grad():
        for i in range(0, len(real_paths), batch_size):
            ...
            real = torch.stack([load_video(p, frames=frames, device=device) for p in real_batch])
            fake = torch.stack([load_video(p, frames=frames, device=device) for p in fake_batch])
